 ...
 *
 * "use client" (tab state) means this can't call cookies() itself -- the
 * parent page.tsx (a Server Component) resolves the dictionary and passes
 * it down as `t`.
 */
...
type TabId = "summary" | "skills" | "companies";

export default function JobMarketRadarReport({ t }: { t: Dictionary["jobMarketRadarReport"] }) {
    ...
    <div className="min-h-screen bg-brand-50 text-brand-950">
      ...
      <header className="bg-white border-b border-brand-200">
        <div className="px-6 py-3 flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-brand-900 font-semibold leading-tight">{t.headerTitle}</p>
      ...
      <main className="p-6 max-w-6xl mx-auto">
        {tab === "summary" && (
          <div className="space-y-6">
            <div className="grid grid-cols-3 gap-4">
              <Card>
                <p className="text-brand-500 text-sm mb-1">{t.kpis.totalPostings}</p>
                <p className="text-3xl font-bold text-brand-900">110</p>
              </Card>
              <Card>
                <p className="text-brand-500 text-sm mb-1">{t.kpis.confirmedDataRoles}</p>
                <p className="text-3xl font-bold text-brand-900">73</p>
              </Card>
              <Card>
                <p className="text-brand-500 text-sm mb-1">{t.kpis.enrichmentCoverage}</p>
                <p className="text-3xl font-bold text-brand-900">100%</p>
              </Card>
            </div>

            <div className="grid md:grid-cols-2 gap-4">
              <Card title={t.postingsByDateTitle}>
                <svg viewBox={`0 0 ${chartW} 210`} className="w-full h-auto">
                  ...
                  {trendPoints.map((p, i) => (
                    <circle key={i} cx={p.x} cy={p.y} r={3} fill={SENIOR_COLOR}>
                      <title>{`${p.v} ${t.postingsTooltipSuffix}`}</title>
                    </circle>
                  ))}
                  {t.trendAxisLabels.map((label, i) => (
                    <text
                      key={label}
                      x={10 + (i * (chartW - 20)) / (t.trendAxisLabels.length - 1)}
                      y={205}
                      fontSize={10}
                      fill="#5a7a8c"
                      textAnchor={i === 0 ? "start" : i === t.trendAxisLabels.length - 1 ? "end" : "middle"}
                    >
                      {label}
                    </text>
                  ))}
                ...
                <p className="text-xs text-brand-400 mt-2">
                  {t.postingsByDateCaption}
                </p>
              </Card>

              <Card title={t.postingsBySeniorityTitle}>
        ...
        {tab === "skills" && (
          <Card title={t.postingsBySkillTitle}>
        ...
        {tab === "companies" && (
          <div className="space-y-6">
            <Card title={t.postingsByCompanyTitle}>
              <BarList data={companiesTop} />
            </Card>
            <Card title={t.jobTitlesCardTitle}>
